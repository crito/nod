...
(function() {
  var e, each, every, flatten, isType, map, nativeEvery, nativeForEach, nativeMap, nativeReduce, nod, previousnod, reduce, root, types, vowels, _, _each, _every, _flatten, _map, _reduce,
    __slice = [].slice,
  ...
  _flatten = function(ary) {
    return _reduce(ary, function(memo, value) {
      if (isType(value, 'Array')) {
        memo = memo.concat(_flatten(value));
      } else {
        memo.push(value);
      }
      return memo;
  ...
  nod.checks.prop = function() {
    ...
    f = function(obj) {
      ...
      result = !isType(obj, 'Object') ? (errors.push('not an object'), false) : !obj.hasOwnProperty(name) ? (errors.push([name, 'not found'].join(': ')), false) : reduce(validators, function(memo, v) {
        var run;
        run = v(obj[name]);
        if (run.length > 0) {
          memo = false;
